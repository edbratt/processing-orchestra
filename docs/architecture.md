...
This diagram focuses on the user-facing lifecycle: loading the browser controls, opening a session, sending per-user input, and cleaning up that user's state when the page closes.

```mermaid
flowchart LR
    subgraph Users["Many Users"]
        UA["User A<br/>browser tab"]
        UB["User B<br/>browser tab"]
        UN["User N<br/>browser tab"]
    end
    ...
    UB --> Static
    UN --> Static
    Controls -->|"JSON on interaction"| WSH
    Streams -->|"motion events + binary audio stream"| WSH
    Sessions --> Draw
    Queue --> Draw
    Audio --> Draw
...
Key details:
- Every browser tab gets its own Helidon session, so user data stays separated by `sessionId`.
- Control input is event-driven, while motion and audio can arrive continuously after the user grants permission.
- The sketch itself is singular: one `ProcessingSketch` instance processes all currently active session data on every frame.
- Sketch-side user state is created lazily when a session first contributes input or audio, then removed when that session closes.
- A sketch author can keep each user's data independent, combine multiple users' data, or do both inside the `draw()` loop.
